...
func initApp(confServer *conf.Server, registry *conf.Registry, confData *conf.Data, logger log.Logger, tracerProvider *trace.TracerProvider) (*kratos.App, func(), error) {
	...
	momentRepo := data.NewMomentRepo(dataData, logger)
	momentUseCase := biz.NewMomentUseCase(momentRepo, logger)
	momentInterface := service.NewMomentInterface(userUseCase, cardUseCase, tagUseCase, momentUseCase, logger)
